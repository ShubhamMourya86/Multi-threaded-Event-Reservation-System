...
void inquireEvent(int eId, int tId) {
  ...
  if (availableSeats[eId] == 0)
    printf("\n\nInquire, Thread - %d, Event - %d : The event is housefull..!",
           tId, eId);
  else
    printf("\n\nInquire, Thread - %d, Event - %d : %d seats available.", tId,
           eId, availableSeats[eId]);
...
int bookEvent(int eId, int seatsToBook, int tId) {
  ...
  if (seatsToBook > availableSeats[eId]) {
    printf("\n\nBooking, Thread - %d, Event - %d : Not enough seats to book %d "
           "seats!",
           tId, eId, seatsToBook);
    pthread_mutex_unlock(&seatMutex);
    return -1;
  ...
  printf("\n\nBooking, Thread - %d, Event - %d : Booked %d seats", tId, eId,
         seatsToBook);
...
int cancelEvent(int tId, vector<pair<int, int>> &bookings) {
  ...
  pthread_mutex_lock(&seatMutex);
  printf(
      "\n\nCancel, Thread - %d, Event - %d : Canceled %d seats for the event.",
      tId, eId, numOfSeats);
